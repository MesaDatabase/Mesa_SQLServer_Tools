...
while exists (select * from @db1 db1)
begin
	set @db = (select top 1 DbName from @db1 as db1)
	set @sql1 = 'exec msdb.dbo.sp_dbmmonitorresults @database_name = ''' + @db + ''';'
	
	insert into #dbm
	exec(@sql1)

	delete from @db1 where DbName = @db
end	
...
select 
  ...
  TimeBehind
from #dbm as dbm
  join sys.database_mirroring as dm1 on dbm.DatabaseName =DB_NAME(dm1.database_id)
drop table #dbm
...
AS (SELECT wait_type, wait_time_ms/ 1000.0 AS [WaitS],
    ...
    WHERE [wait_type] NOT IN (
        ...
        N'XE_DISPATCHER_WAIT', N'XE_TIMER_EVENT',N'QDS_ASYNC_QUEUE',N'XTP_PREEMPTIVE_TASK')
...
--CHECK #16: Top Waits (Current)
select SUM(wait_duration_ms) as [Wait Duration], wait_type
...
[wait_type] NOT IN (
        ...
        N'SLEEP_TEMPDBSTARTUP', N'SNI_HTTP_ACCEPT', N'SP_SERVER_DIAGNOSTICS_SLEEP',
              ...
              N'WAIT_FOR_RESULTS', N'WAITFOR', N'WAITFOR_TASKSHUTDOWN', N'WAIT_XTP_HOST_WAIT',
              N'WAIT_XTP_OFFLINE_CKPT_NEW_LOG', N'WAIT_XTP_CKPT_CLOSE', N'XE_DISPATCHER_JOIN',
        N'XE_DISPATCHER_WAIT', N'XE_TIMER_EVENT',N'PREEMPTIVE_HADR_LEASE_MECHANISM',N'QDS_ASYNC_QUEUE',N'XTP_PREEMPTIVE_TASK')
Group by wait_type
Order by [Wait Duration] DESC

--CHECK #17: Authentication mode is mixed
select @@servername, 
...
--CHECK #18: Last Statistics Update
...
--CHECK #20: Tlog utilization
DBCC SQLPERF(LOGSPACE)

--CHECK #21: Disk utilization
...
--CHECK #22: Global trace flags enabled
DBCC TRACESTATUS (-1);

--CHECK #23: Windows error log manual check
