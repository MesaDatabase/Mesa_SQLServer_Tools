...
create table #db (DatabaseName varchar(100))
declare @sql0 varchar(2000)
declare @ver varchar(25)
declare @ver2 int
set @ver = (select master.dbo.fsplit('.',cast(SERVERPROPERTY('productversion') as varchar(25)),1))
set @ver2 = (select cast(@ver as int))

if  @ver2 < 11 and @ver2 > 8
begin
  set @sql0 = 'insert into #db
select s1.name
...
select 
  ...
  master.dbo.fSplit(',',Output,4) as DriveLabelClean,
...
if exists (select * from @t5b)
begin
  select 8 as ScriptNum, @@servername as SQLInstance, * from @t5b
end
else
begin
  select 8 as ScriptNum, @@servername as SQLInstance, 'No aliases configured' as Notes
end
...
from master.sys.databases as s1
  left join sys.dm_hadr_database_replica_states as h1 on s1.database_id = h1.database_id
where state_desc = 'ONLINE'
  and s1.name not in ('master','msdb')
  and isnull(h1.is_primary_replica,1) = 1
...
begin
  select 22 as ScriptNum, @@servername as SQLInstance, 'No credentials configured' as Notes
end

--SCRIPT #23: PROXIES
if exists (select * from msdb.dbo.sysproxies)
begin
...
begin
  select 23 as ScriptNum, @@servername as SQLInstance, 'No proxies configured' as Notes
end
...
begin
  select 24 as ScriptNum, @@servername as SQLInstance, 'No assymetric keys configured' as Notes
end


--SCRIPT #25: TDE SYMETRIC KEYS
if exists (select * from master.sys.symmetric_keys where name != '##MS_ServiceMasterKey##')
begin
...
begin
  select 25 as ScriptNum, @@servername as SQLInstance, 'No symmetric keys configured' as Notes
end
...
if exists (select * from msdb.dbo.log_shipping_secondary)
begin
...
begin
  select 27 as ScriptNum, @@servername as SQLInstance, 'No logshipping secondary configured' as Notes
end

--SCRIPT #28: MIRRORING
if exists (select * from msdb.sys.database_mirroring where mirroring_guid is not null)
begin
...
begin
  select 28 as ScriptNum, @@servername as SQLInstance, 'No mirroring configured' as Notes
end
