...
#  $SqlInstances = (Get-Content C:\Users\a-Renee.Jaramillo\Documents\sqlinstances.txt)
  ...
  $InstanceId = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\Instance Names\SQL' -Name $InstanceName).$InstanceName

    # Start DAC connection to SQL Server
    # Default instance MSSQLSERVER -> instance name cannot be used in connection string
    if ($InstanceName -eq "MSSQLSERVER") {
#      $RegPath = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\$InstanceId")
      $ClusterName = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\$InstanceId\Cluster\" -Name ClusterName).ClusterName
      $ConnString = "Server=ADMIN:$ClusterName;Trusted_Connection=True"
    }
    else {
#      $RegPath = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\$InstanceId").
      $ClusterName = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\$InstanceId\Cluster\" -Name ClusterName).ClusterName
      $ConnString = "Server=ADMIN:$ClusterName\$InstanceName;Trusted_Connection=True"
    }
    ...
    #  $RegPath = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\Instance Names\sql\").$InstanceId
      [byte[]]$Entropy = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\$InstanceId\Security\" -Name Entropy).Entropy
