...
create table DBA.dbo.tmp_BackupCopy (output varchar(max))
...
declare @t0 table (Output varchar(100))
declare @t1 table (DbName varchar(100))
declare @t2 table (DbName varchar(100), BackupSetId int, FileSizeGB decimal(18,2), BakFilePath varchar(1000))
declare @t3 table (BakFilePath varchar(1000))
...
begin
  ...
  begin
    set @sql = 'exec xp_cmdshell ''dir ' + @destPath + '\' + @database + '*.' + @ext + ''''
    insert into @t3 exec(@sql)
	set @sql = 'exec xp_cmdshell ''del /Q /F ' + @destPath + '\' + @database + '*.' + @ext + ''''
  end
  else 
  begin
    set @sql = 'exec xp_cmdshell ''dir ' + @destPath + '\*.' + @ext + ''''
    insert into @t3 exec(@sql)
    set @sql = 'exec xp_cmdshell ''del /Q /F ' + @destPath + '\*.' + @ext + ''''
  end
...
set @sql = 'exec xp_cmdshell ''dir ' + @destPath + ' | findstr "bytes" | findstr "free"'''
insert into @t0
exec(@sql)
...
begin
  ...
  begin
    ...
    begin
      set @sql = 'exec xp_cmdshell ''xcopy ' + @path + ' ' + @destPath + ' /i /y'''
      print(@sql)
    end
...
set @sql = 'exec xp_cmdshell ''dir ' + @destPath + ''''
insert into DBA.dbo.tmp_BackupCopy
exec(@sql)
end
